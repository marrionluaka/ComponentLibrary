...
const Template = () => ({
      ...
      <div class="flex gap-x-2">
        <Tab :id="0" :selected="true">
          <template #default="{ active, setActiveTabID }">
            <button :class="getClasses(active)" @click="setActiveTabID(0)">HTML</button>
          </template>
        </Tab>

        <Tab :id="1">
          <template #default="{ active, setActiveTabID }">
            <button :class="getClasses(active)" @click="setActiveTabID(1)">CSS</button>
          </template>
        </Tab>

        <Tab :id="2">
          <template #default="{ active, setActiveTabID }">
            <button :class="getClasses(active)" @click="setActiveTabID(2)">JavaScript</button>
          </template>
        </Tab>

        <Tab as="button" :class="[tabClass, 'bg-gray-100 text-gray-500']" :id="3" :disabled="true">
          Haskell
        </Tab>
      </div>
  ...
  setup() {
    const tabClass = ref('border border-gray-300 rounded-md py-1 px-3 text-sm')

    function getClasses(active) {
      return [
        `appearance-none ${tabClass.value}`,
        active ? 'bg-indigo-500 border-indigo-500 text-white' : 'text-gray-900 border-gray-300'
      ]
    }

    return { tabClass, getClasses }
  }
})
